...
Pour cela, nous mobilisons des techniques de NLP (\textit{Natural Language Processing}), ainsi que des méthodes de machine learning.
...
La littérature sur le domain-adaptive pretraining montre aussi qu'un encoder générique peut gagner en qualité lorsqu'il est continué sur des textes proches de la tâche cible \cite{gururangan2020dont}. Cette idée motive une perspective directe pour ce projet : adapter les modèles au vocabulaire e-commerce avant le fine-tuning ou l'inference zéro-shot.
...
Avant de traiter séparément le Niveau 1 supervisé et les niveaux L2--L7 non supervisés, nous construisons une représentation enrichie commune des catégories. Un nom court comme \textit{hardware} ou \textit{office supplies} ne suffit pas toujours à couvrir le vocabulaire des produits. Nous représentons donc chaque catégorie par plusieurs prototypes : nom canonique, chemin taxonomique, enfants, descendants et variantes lexicales générées par LLM.

Formellement, une catégorie $c$ est représentée par :
\begin{equation}
    \mathcal{P}_c =
    \{p_c^{\text{name}}, p_c^{\text{path}}, p_c^{\text{children}},
    p_c^{\text{desc}}, p_{c,1}^{\text{lex}}, \ldots, p_{c,r}^{\text{lex}}\}.
\end{equation}
...
\begin{equation}
    S(x,c)=\max_{p\in\mathcal{P}_c}\cos(f(x),f(p))
\end{equation}
revient donc à approximer la similarité entre le produit et le manifold $\mathcal{M}_c$ par la similarité au prototype le plus proche. En termes géométriques, on remplace une distance à un point par une distance à un ensemble de points. Cela améliore le recouvrement local du manifold : un produit peut être reconnu même s'il emploie un vocabulaire proche d'un enfant ou d'une variante lexicale plutôt que du nom canonique.
...
Concrètement, le script \texttt{scripts/generate\_category\_enrichment.py} reconstruit la taxonomie depuis \texttt{taxonomy.txt}, calcule pour chaque nœud son chemin, son parent, ses enfants directs et ses descendants, puis exporte pour chaque niveau deux fichiers : \texttt{level\_L\_categories.csv} et \texttt{level\_L\_reference\_texts.csv}. Ces fichiers séparent les métadonnées de catégorie et les textes de référence directement utilisables par le semantic retrieval.
...
Le Niveau 1 est le seul niveau annoté pour tous les produits. Il sert donc de point d'entrée au pipeline complet : une erreur au Niveau 1 contraint ensuite tout le sous-arbre exploré en L2--L7. Nous n'entraînons pas un flat classifier, mais un metric space où produits et \hyperref[chap:category-enrichment]{prototypes de catégories enrichies} sont comparables par cosine similarity.
...
\begin{table}[H]
    ...
    \begin{tabular}{lrrrrr}
        ...
        \texttt{BAAI/bge-small-en-v1.5} & 74 & 33M & 64{,}30 & 76{,}56 & 53{,}86 \\
...
\begin{figure}[H]
    ...
    \caption{Évolution de la training loss lors du fine-tuning supervisé du Niveau 1 avec sampling tempéré $\alpha=0{,}5$.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth]{figures/l1_category_accuracy_sampling_comparison.pdf}
    \caption{Comparaison des accuracies top-1 par catégorie pour les cinq stratégies de sampling testées.}
    \label{fig:l1-category-accuracy-sampling-comparison-main}
\end{figure}

La figure \ref{fig:l1-category-accuracy-sampling-comparison-main} confirme que le gain du sampling tempéré est surtout visible sur les catégories minoritaires. Les stratégies trop équilibrées peuvent faire chuter des catégories fréquentes, tandis que les stratégies trop proches du sampling naturel conservent les défauts initiaux. Le sampling tempéré agit donc comme une régularisation de la distribution des batches.
...
\begin{table}[H]
    ...
    \begin{tabular}{lrrrrrr}
        ...
        \midrule
        \texttt{Qwen3-Embedding-0.6B} & 1024 & 163{,}2 min & 17{,}0 & 650{,}6 MB & 7659{,}0 MB & 15\,175{,}3 M \\
        \texttt{Jasper-Token-Compression-600M} & 2048 & 121{,}6 min & 22{,}8 & 1301{,}1 MB & 2774{,}9 MB & 30\,350{,}6 M \\
        \bottomrule
    \end{tabular}
    }
\end{table}

Jasper encode plus vite dans notre run Kaggle, mais ses embeddings de dimension 2048 doublent la taille mémoire et le coût vectoriel par rapport à Qwen3. Ce point est visible dans la figure \ref{fig:l2-l7-embedding-generation-cost-main}. Pour un catalogue très volumineux, le coût le plus important n'est pas seulement le temps initial d'encodage : c'est aussi la taille des matrices à stocker, charger, transférer et comparer à chaque étape de retrieval. Comme la contrainte industrielle porte surtout sur le coût d'inference et le stockage à grande échelle, Qwen3 est le choix le plus cohérent pour la suite.
...
\subsection{Pipeline 1 : Décodage Hiérarchique Local}
...
\subsection{Pipeline 2 : Clustering Hiérarchique Contraint par la Taxonomie}
...
\subsection{Pipeline 3 : Retrieval de Chemins Globaux}
...
Greedy est le meilleur compromis coût/inference : environ 1{,}14 ms par produit avec Qwen3. Beam + reranker améliore la sélection finale, mais coûte environ 16 ms par produit. La figure \ref{fig:l2-l7-pipeline-runtime-cost-main} rend visible ce compromis : le passage de greedy à beam augmente modérément le coût, tandis que l'ajout d'un reranker change d'ordre de grandeur. Global path est plus diversifié, notamment avec Qwen3, mais sa marge top-1/top-2 reste faible. Les figures et matrices détaillées de divergence sont données en \hyperref[sec:l2-l7-family3-matrices-annex]{annexe}.
...
L'audit NLP clarifie le compromis. Le bi-encoder favorise le global path en score moyen, car ce pipeline optimise directement une similarité produit--chemin global. Cependant, le greedy reste très compétitif en win rate : il arrive souvent en première position parmi les cinq propositions, même si son score moyen n'est pas maximal. Le cross-encoder favorise nettement beam + reranker, ce qui confirme sa meilleure qualité brute ; toutefois cette option est environ quatorze fois plus lente que greedy. Le détail complet de l'audit NLP et le coût du scoring sont reportés en \hyperref[sec:l2-l7-nlp-audit-annex]{annexe}.

\section{Comparaison Globale et Choix Opérationnel}
\label{sec:l2-l7-operational-choice}

Le choix opérationnel retenu est \textbf{Qwen3 + greedy}. Qwen3 réduit le coût vectoriel et la taille mémoire par rapport à Jasper. Greedy est suffisamment cohérent pour constituer une première passe industrielle, avec un coût très faible et une lecture simple. Beam + reranker est conservé comme seconde passe pour les produits incertains : faible marge locale, désaccord fort entre pipelines ou branche L1 connue comme instable. Le clustering reste utile comme audit structurel, et global path comme baseline de diversité.
...
\begin{table}[H]
    \centering
    \caption{Extrapolation du coût d'inference avec Qwen3 + greedy à partir des mesures observées}
    \label{tab:conclusion-scale-up}
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{lrrrr}
        \toprule
        \textbf{Volume} & \textbf{Temps embedding Qwen3} & \textbf{Temps greedy L2--L7} & \textbf{Stockage embeddings Qwen3 fp16} & \textbf{Lecture opérationnelle} \\
        \midrule
        1 million de produits & $\approx$ 16{,}3 h & $\approx$ 19 min & $\approx$ 1{,}9 GB & Faisable sur une seule machine GPU si le traitement est batché \\
        1 milliard de produits & $\approx$ 681 jours & $\approx$ 13{,}2 jours & $\approx$ 1{,}9 TB & Nécessite sharding, parallélisation multi-GPU et stockage distribué \\
        \bottomrule
    \end{tabular}
    }
\end{table}
...
La distribution des langues est maintenant présentée directement dans le \hyperref[chap:descriptive-statistics]{chapitre 3}, car elle justifie le choix d'embedding models anglais compacts.
...
\section{Audit NLP Complet L2--L7}
\label{sec:l2-l7-nlp-audit-annex}

\noindent\hyperref[sec:l2-l7-pipeline-agreement]{Retour vers les résultats synthétiques L2--L7.}

\begin{table}[H]
    \centering
    \caption{Audit NLP complet avec bi-encoder sur 128\,253 produits et 641\,265 paires.}
    \label{tab:l2-l7-full-biencoder-audit-annex}
    \scriptsize
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{lrrrrrr}
        \toprule
        \textbf{Pipeline} & \textbf{Score Qwen} & \textbf{Win Qwen} & \textbf{Rang Qwen} & \textbf{Score Jasper} & \textbf{Win Jasper} & \textbf{Rang Jasper} \\
        \midrule
...
\begin{table}[H]
    \centering
    \caption{Audit NLP complet avec cross-encoder sur 128\,253 produits et 641\,265 paires.}
    \label{tab:l2-l7-full-crossencoder-audit-annex}
    \scriptsize
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{lrrrrrr}
        \toprule
...
\begin{table}[H]
    \centering
    \caption{Coût de l'audit NLP complet sur Kaggle.}
    \label{tab:l2-l7-full-nlp-audit-cost-annex}
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{llrrrrr}
        \toprule
        \textbf{Embedder évalué} & \textbf{Scorer} & \textbf{Produits} & \textbf{Paires} & \textbf{Temps} & \textbf{Débit} & \textbf{VRAM réservée} \\
        \midrule
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.48\textwidth]{figures/l2_l7_nlp_audit_full/l2_l7_nlp_audit_win_rates.pdf}
    \hfill
    \includegraphics[width=0.48\textwidth]{figures/l2_l7_nlp_audit_full/l2_l7_nlp_audit_mean_scores.pdf}
    \caption{Audit NLP complet : win rates et scores moyens par pipeline.}
    \label{fig:l2-l7-full-nlp-audit-annex}
\end{figure}
...
La figure \ref{fig:l1-category-accuracy-sampling-comparison} présente le détail par catégorie des cinq stratégies de sampling testées pour le fine-tuning du Niveau 1. Elle est placée en annexe et affichée en grand format afin de conserver la lisibilité des noms de catégories et des valeurs d'accuracy.

\begin{figure}[p]
    \centering
    \includegraphics[width=0.98\textwidth,height=0.78\textheight,keepaspectratio]{figures/l1_category_accuracy_sampling_comparison.pdf}
    \caption{Comparaison des accuracies top-1 par catégorie pour les cinq stratégies de sampling.}
