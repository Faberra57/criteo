...
class PipelineConfig:
    input_path: str = "dataset/preprocessed_lv2.parquet"
    input_format: str = "parquet"
    output_path: str = "data/l1_to_l7_beam_predictions.csv"
    enrichment_dir: str = "dataset/category_enrichment"
    l1_model_path: str = str(DEFAULT_L1_RUN_DIR / "best_model")
    l1_run_dir: str = str(DEFAULT_L1_RUN_DIR)
    deep_model_name: str = "BAAI/bge-base-en-v1.5"
    prototype_cache_dir: Optional[str] = "data/prototype_embedding_cache"
...
def parse_args() -> tuple[PipelineConfig, bool]:
    ...
    return (
        PipelineConfig(
            ...
            device_l1=args.device_l1,
            device_deep=args.device_deep,
...
def run_pipeline(config: PipelineConfig, *, dry_run: bool = False) -> pd.DataFrame | None:
    ...
    l1_metadata = load_category_metadata(Path(config.enrichment_dir), min_depth=1, max_depth=1)
    deep_metadata = load_category_metadata(
        Path(config.enrichment_dir), min_depth=1, max_depth=config.max_depth
    )
    parent_to_children = build_parent_to_children(deep_metadata)
